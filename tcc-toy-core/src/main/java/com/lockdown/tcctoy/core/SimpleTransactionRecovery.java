...
public class SimpleTransactionRecovery implements TccTransactionRecovery {
	...
	private void retryInvokeTransaction(List<TccTransaction> transactions) {
		for(TccTransaction transaction : transactions) {
			try {
				...
				case CONFIRMING:{
					logger.info("{} transaction {} recover confirm  =========== ",LogUtils.LOG_PREFFIX,transaction.transactionId());
					TccTransactionManager.commitTransaction();
					break;
				}
				case CANCELING:{
					logger.info("{} transaction {} recover cancel  =========== ",LogUtils.LOG_PREFFIX,transaction.transactionId());
					TccTransactionManager.rollbackTransaction();
					break;
				}
				case PREPARATION:{
					logger.info("{} transaction {} is preparetion status, try to delete !",LogUtils.LOG_PREFFIX,transaction.transactionId());
					TccTransactionManager.tryToDelete(transaction);
					break;
				}
				default:
					throw new IllegalStateException(LogUtils.LOG_PREFFIX+" can't supported recovery current "+transaction.getTransactionStatus()+" state transaction , id is "+transaction.transactionId());
				}
			}catch(Throwable ex) {
				logger.warn("{}  transaction {}  recover failed ! error : {} ",LogUtils.LOG_PREFFIX,transaction.transactionId(),ex.getMessage());
			}
		}
	...
	private List<TccTransaction> loadRecoveryTransaction(){
		...
		logger.warn(" found error transaction size: {} ",LogUtils.LOG_PREFFIX,errorTransaction.size());
		...
		for(TccTransaction transaction : errorTransaction) {
			try {
				...
				logger.warn("{} transaction {} version mismatch,skip current transaction !",LogUtils.LOG_PREFFIX,transaction.transactionId());
				continue;
			}
			retryTransaction.add(transaction);
		}
		logger.warn("{} recover transaction size: {}",LogUtils.LOG_PREFFIX,retryTransaction.size());
